...
const Classic = ({match}) => {
  ...
  useEffect(  ()  => {
    setLoading(true);
    const ticker = match.params.token+"/"+match.params.base;
    try {
      ...
      res.then((data =>{
        if(data.data.data.length > 0){
          ...
          priceFetch.then((rs =>{
            ...
            setMarket({...market, ['market_address']: data.data.data[0].marketAddress, ['token_address']: data.data.data[0].tokenAddress, ["last_price"]: parseFloat(rs.data.data), ['address']: data.data.data[0].contract_address, ['token']: match.params.token, ['base']: match.params.base});
            setLoading(false);
          }))
